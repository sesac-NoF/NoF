...
# 데이터 가져오기
cursor.execute("SELECT 사건번호,사건명,키워드,키워드임베딩,판례내용이유임베딩 FROM inheritance_embedding")
...
# 판례내용이유임베딩과 키워드임베딩을 문자열에서 NumPy 배열로 변환
df_inheritance['판례내용이유임베딩'] = df_inheritance['판례내용이유임베딩'].apply(
    lambda x: np.array(ast.literal_eval(x)) if pd.notnull(x) else None
)
df_inheritance['키워드임베딩'] = df_inheritance['키워드임베딩'].apply(
    lambda x: np.array(ast.literal_eval(x)) if pd.notnull(x) else None
)
